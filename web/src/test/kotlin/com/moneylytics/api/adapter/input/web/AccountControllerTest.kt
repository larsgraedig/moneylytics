...
class AccountControllerTest {
    ...
    private val controller = AccountController(getAccountsUseCase, saveAccountUseCase, deleteAccountUseCase, resolveUserUseCase)
    ...
    fun `should return 400 when IBAN is blank on createAccount`() =
        runTest {
            val response = controller.createAccount(SaveAccountRequest(iban = "  ", name = "Giro"), principal)
    ...
    fun `should trim IBAN and name before passing to use case`() =
        runTest {
            ...
            whenever(saveAccountUseCase.saveAccount("DE00TEST", "Giro", userId)).thenReturn(saved)

            val response = controller.createAccount(SaveAccountRequest(iban = " DE00TEST ", name = " Giro "), principal)

            assertThat(response.statusCode).isEqualTo(HttpStatus.OK)
            verify(saveAccountUseCase).saveAccount("DE00TEST", "Giro", userId)
        }
    ...
    fun `should map latestTransactionDate to string in getAccounts`() =
        runTest {
            ...
            val response = controller.getAccounts(principal)
    ...
    fun `should return 204 on deleteAccount`() =
        runTest {
            val response = controller.deleteAccount("DE00TEST", principal)

            assertThat(response.statusCode).isEqualTo(HttpStatus.NO_CONTENT)
            verify(deleteAccountUseCase).deleteAccount("DE00TEST", userId)
        }
}
