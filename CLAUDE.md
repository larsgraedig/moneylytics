...
- Liquibase changesets are numbered sequentially (`0001`, `0002`, …). Check the highest existing number in `web/src/main/resources/db/changelog/changes/` before creating a new one. The changeset `id` must be a quoted, four-digit string matching the file's number (e.g. `id: "0007"`), incrementing by one from the previous changeset.
